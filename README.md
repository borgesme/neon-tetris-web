...
游戏规则位于 `src/game`，不依赖 React、DOM、Canvas 或浏览器存储。React 负责组合页面状态和交互，`GameCanvas` 只从当前游戏状态绘制棋盘、活动块、锁定块和 Ghost Piece。音效由 `src/audio` 和 `src/hooks/useSoundEffects.ts` 在 UI 层处理，避免把浏览器副作用放入核心规则。这样的分层让核心规则可以通过 Vitest 单独验证，也便于未来增加更多主题或在线排行榜。
